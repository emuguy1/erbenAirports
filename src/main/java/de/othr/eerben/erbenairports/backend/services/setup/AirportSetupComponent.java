    ...
    boolean setup() throws AirportException {
        try {
            if (airportRepo.existsAirportByAirportcode("MUC")) {
                logger.info("Airport Setup skipped, because Airport MUC exists!");
                return true;
            }
            ...
            logger.error("Airport Setup failed! Couldnt complete Airport Setup! Error Message:" + e.getMessage());
